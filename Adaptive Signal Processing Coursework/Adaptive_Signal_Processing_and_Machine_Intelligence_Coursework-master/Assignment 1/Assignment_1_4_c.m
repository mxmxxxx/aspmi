...
for k=1:length(N)
    ...
    for i=1:length(order)
        ...
        aic_aryule (i) = log10(err(i)) + (2*i)/N(k); %Akaike information criterion
        aicc_aryule (:,i) =  aic_aryule(i) + (((2*i)*(i+1))/(N(k)-i-1)); %Corrected AIC
        
        if order(i)==2 || order(i)==4 || order(i)==9 || order(i)==14
            figure(1);
            subplot(2,2,ind)
            ...
            xlim([0 0.5])
            title(['PSD estimate-AR(' num2str(order(i)) ') model (N=9500)'],'FontSize',11)
            legend(['True PSD'],['order ' num2str(order(i))],'FontSize',11)
            ind=ind+1;
        end
    end
    ...
    title(['MSE vs model order(N=',num2str(N(k)),')'],'FontSize',11)
